...
用法: python3 codex-sync-merge.py <local_file> <remote_file> <output_file>
输出到 stdout: NO_REMOTE_NEW | NO_LOCAL_NEW | MERGED:local_new=N remote_new=N
"""
import json
import sys
...
def main():
    ...
    marker = json.dumps({
        ...
        "payload": {
            "role": "user",
            "content": [{"type": "input_text", "text": f"[_auto_merged_from_remote] 远程 VSCode 会话新增 {len(remote_items)} 条对话"}]
        }
    }) + "\n"
